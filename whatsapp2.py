...
while True:
    ...
    for id, username, pass_date, num in PassData:
        ...
        if str(pass_date.time()) >= "08:00:00" and str(pass_date.time()) <= "11:00:00":
            msg = """تم دخول طالب  {username} الى المدرسة
            {pass_date}"""
        elif str(pass_date.time()) >= "11:00:00" and str(pass_date.time()) <= "16:00:00":
            msg = """تم خروج طالب {username} من المدرسة
            {pass_date}"""
        else:
            msg = """تم خروج طالب {username} من المدرسة__
    {pass_date}"""
        
        pyperclip.copy(msg.format(username=username, pass_date=str(pass_date)))
